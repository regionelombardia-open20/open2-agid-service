...
class AgidServiceSearch extends AgidService implements CmsModelInterface {
    ...
    public function search($params, $queryType = NULL, $limit = NULL, $onlyDrafts = false, $pageSize = NULL) {
        ...
        //ricerca solo per contenuti che posso vedere AGID
        if (!\Yii::$app->getUser()->can('AGID_SERVICE_ADMIN')) {
            ...
            $query->andWhere(['agid_service.agid_service_type_id' => $ids,]);
        }
        ...
        $query->joinWith('agidServiceContentType', false);
        $query->joinWith('agidServiceStatus', false);
        $query->joinWith('agidServiceType', false);
        $query->joinWith('agidUoManager', false);
